...
*The guide covers only the creation of basic AWS infrastructure. It does not cover the complete ESG configuration.*
___
...
Use the same prefix for all infrastructure element names.
Add a descriptive suffix, for example, **esg-qa**-<u>cluster</u>.
This convention identifies ESG elements and lets you limit IAM rules to specified resources.
...
This step creates an EC2 instance for the ESG server. Skip this step if you already have the specified instance.
...
3. Attach policy from template provided in file: ==`esg-node-role-policy.json`==.
   ...
   Replace `<ZEBRUNNER_ENV>` with your prefix.
   Alternatively, replace the complete cluster name with your custom name.
...
2. Create new launch template (Suggested name: ==esg-qa-lt==).
   ...
   * Instance type: Do not include it in the launch template.
   * Key pair: Add yours if you want ssh access to worker nodes.
   * Subnet and Availability Zone: Do not include them in the launch template.
...
* If private subnets use a NAT gateway, S3 access works through NAT.
  Add an S3 Gateway VPC endpoint to reduce NAT traffic and cost.
...
3. Attach policy from template provided in file: ==`esg-task-role-policy.json`==.
   Replace `<ZEBRUNNER_ENV>` with your prefix.
   Alternatively, replace the complete bucket name with your custom name.
4. Check/modify trust relations to be as provided bellow:
```
{
    "Version": "2012-10-17",
