...
* IMAGE_REPOSITORIES - Repositories with supported browsers. Valid names: `chrome`, `firefox`, `edge`, `windows-chrome`, `windows-edge`, `windows-firefox`, `cypress-chrome`, `cypress-chromium`, `cypress-edge`, `cypress-firefox`, `playwright`, `chrome-auto-update`, `edge-auto-update`, `firefox-auto-update`. The default list includes `playwright`.
